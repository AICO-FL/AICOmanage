...
import { Action, ActionSchema, type ActionFormData } from '../../types/action';
...
export function EditActionModal({ action, isOpen, onClose }: EditActionModalProps) {
  const [keywords, setKeywords] = useState<string[]>(action.keywords);
  const [newKeyword, setNewKeyword] = useState('');
  const [error, setError] = useState<string | null>(null);

  const { updateAction, isUpdateLoading } = useActions();
  ...
  const onSubmit = async (data: ActionFormData) => {
    try {
      ...
      await updateAction(action.id, data, keywords);
      onClose();
    } catch (err: any) {
      setError(err.message || 'アクションの更新に失敗しました');
    }
  };
    ...
    <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl">
        <div className="flex items-center justify-between p-4 border-b">
          <h3 className="text-lg font-medium">アクションの編集</h3>
          <button
            onClick={onClose}
